 ...
 *
 * ## Why one form, not six
 *
 * The catalogue has six. This ships **12-C, Arbitration**, alone, because it
 * is the one that meets the milestone's exit criterion — *a player can show
 * you an item they built and explain why it is theirs* — and because it needs
 * nothing that does not already exist. 7-A needs hidden clauses; 44 needs
 * provenance; 3-B needs vacant slots, which today's rolls almost never leave.
 * Each of those is a change to what drops, and a change to what drops is a
 * re-measurement. This one is not.
 *
 * The rails, though, are the whole system's rails: a filing is queued with a
 * resolution tick, resolves inside the tick loop like everything else, and is
 * seeded so that replaying the span cannot change the outcome. Adding 3-B, 19
 * and the rest is authoring after this.
 */

export type FormId = '12-C';
